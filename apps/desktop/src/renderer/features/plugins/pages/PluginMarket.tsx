...
export function PluginMarket() {
  ...
  const fetchMarket = async (silent = false) => {
    ...
    setError('')
    try {
      const result = await pluginService.marketList()
      if (result.success) {
  ...
  useEffect(() => {
    ...
    const unsubChanged = pluginService.onPluginsChanged(() => {
      void refreshInstalled()
    })
    const unsubProgress = pluginService.onDownloadProgress((payload) => {
      setDownloads((prev) => ({
        ...prev,
  ...
  const install = async (plugin: { name: string; downloadUrl?: string }) => {
    ...
    const result = await pluginService.installFromMarket({
      name: plugin.name,
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                setPulling(true)
                pluginService.marketClearCache().then(() => {
                      ...
                      {isInstalled ? (
                        <button
                          onClick={(e) => {
                            ...
                            if (p) pluginService.launch(p.path)
                          }}
