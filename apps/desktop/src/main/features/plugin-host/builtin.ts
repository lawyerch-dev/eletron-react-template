...
/**
 * 解析内置插件根目录。
 * 打包模式：resources/plugins/（extraResources 自 apps/desktop/plugins）
 * 开发模式：apps/desktop/plugins/（APP_ROOT = apps/desktop）
 */
export function resolveBuiltinPluginsRoot(): string {
  ...
  const candidates = [path.join(appRoot, 'plugins'), path.join(appRoot, '..', '..', 'plugins')]
