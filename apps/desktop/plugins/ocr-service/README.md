...
```bash
uv run --directory apps/desktop/plugins/ocr-service/scripts python rapidocr_runner.py '{"probe": true}'
```
...
```bash
cd apps/desktop/plugins/ocr-service && ./scripts/setup_rapidocr.sh   # uv venv 到插件目录
