...
description: Anti-slop frontend skill for **landing pages, portfolios, and redesigns only**. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check. Do NOT load for dashboards, data tables, multi-step product UI, or Electron desktop tool interfaces.
---
