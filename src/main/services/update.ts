...
export function update(win: Electron.BrowserWindow) {
  ...
  autoUpdater.on('update-available', (arg: UpdateInfo) => {
    win.webContents.send(IpcChannel.UpdateCanAvailable, {
      update: true,
  ...
  ipcMain.handle(IpcChannel.CheckUpdate, async () => {
    if (!app.isPackaged) {
  ...
  ipcMain.handle(IpcChannel.StartDownload, (event: Electron.IpcMainInvokeEvent) => {
    ...
    startDownload(
      (error, progressInfo) => {
        if (error) {
          ...
          event.sender.send(IpcChannel.UpdateError, { message: error.message, error })
        } else {
          // feedback update progress message
          event.sender.send(IpcChannel.DownloadProgress, progressInfo)
        }
      },
      () => {
        isDownloading = false
        // feedback update downloaded message
        event.sender.send(IpcChannel.UpdateDownloaded)
      },
    )
  })

  // Cancel downloading
  ipcMain.handle(IpcChannel.CancelDownload, () => {
    cancellationToken.cancel()
    cancellationToken = new updater.CancellationToken()
  })

  // Install now
  ipcMain.handle(IpcChannel.QuitAndInstall, () => {
