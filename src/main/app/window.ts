...
export function getMainDist(): string {
  return path.join(getAppRoot(), 'out/electron')
}

export function getRendererDist(): string {
  return path.join(getAppRoot(), 'out/renderer')
}
...
export function getIndexHtmlPath(): string {
  // 入口在 src/renderer/index.html，构建后保留相对路径
  return path.join(getRendererDist(), 'src/renderer/index.html')
}
