...
```bash
uv run --directory src/plugins/ocr-service/scripts python rapidocr_runner.py '{"probe": true}'
```
...
```bash
cd src/plugins/ocr-service && ./scripts/setup_rapidocr.sh   # uv venv 到插件目录
